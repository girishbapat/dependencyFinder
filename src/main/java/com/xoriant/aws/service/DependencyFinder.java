...
public interface DependencyFinder {
	/**
	 * API to find dependencies for given table.
	 * This api first performs following validations for given table name:<br>
	 * 1. Whether incremental data is available for current table and all the dependent tables or not<br>
	 * 2. Whether if there is any circular dependency is present
	 * <br><strong>If any of the validations fails, it does not proceed to find dependencies <br>
	 ...
	 * 1. Whether incremental data is available for current table and all the dependent tables or not<br>
	 * 2. Whether if there is any circular dependency is present.
	 * It returns consolidated validation issues in List.
	 *
	 * @param nameOfTable
	...
	/**
	 * This API is same as basic <strong> Queue<String> findDependencies(String nameOfTable )throws RuntimeException;</strong> except<br>
	 * this API accepts boolean value, depending upon same end user can decide to perform validations or not
	 * default is true
	 * @param nameOfTable
